...
export const api = {
  ...
  // 批量检查所有密钥
  checkAllKeys: () =>
    req<{ ok: boolean; total: number; message: string }>('/api/keys/check-all', {
      method: 'POST',
    }),
  // 重置所有密钥健康状态
  resetKeyHealth: () =>
    req<{ ok: boolean; keysReset: number; modelsReset: number }>('/api/keys/reset-health', {
      method: 'POST',
    }),
  // 用真实请求测试密钥
  testKey: (id: number, model?: string) =>
    req<{ ok: boolean; status: number; latency: number; healthStatus: string; response: string | null }>(`/api/keys/${id}/test`, {
      method: 'POST',
  ...
  // About (公开端点,无需鉴权 — 仅返回 name + version)
  getAbout: () => req<any>('/api/about'),
  // About 详情(需鉴权 — 返回完整运营信息、统计、变更日志)
  getAboutDetail: () => req<any>('/api/about/detail'),

  // Theme — 全局主题持久化(保存到后端 D1,所有页面 + landing page 共享)
  getTheme: () => req<{ accent: string; glow: number; glass: number; gradient: number }>('/api/theme'),
  setTheme: (prefs: { accent: string; glow: number; glass: number; gradient: number }) =>
    req<{ success: boolean }>('/api/settings/theme', {
      method: 'PUT',
