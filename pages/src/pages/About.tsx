...
export function AboutPage() {
  ...
  useEffect(() => {
    ...
    const fetchInfo = async () => {
      try {
        const data: AboutInfo = await api.getAboutDetail();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold gradient-text">{t('about.title')}</h1>
        <p className="text-text-secondary text-sm mt-1">{t('about.subtitle')}</p>
      </div>

      {err && (
        <div className="rounded p-3 text-sm alert-danger">
          {err}
        </div>
      )}
      ...
      {!info ? (
        ...
        <>
          {/* 运行时间卡片 (高亮) */}
          <div
            className="rounded-lg p-6 alert-info galaxy-fade-in border"
                  ...
                  <div key={p.platform} className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex-1 mx-3 h-1.5 rounded-full bg-bg-tertiary overflow-hidden">
                      <div
                        ...
                        style={{ width: `${p.total ? (p.enabled / p.total) * 100 : 0}%`, boxShadow: '0 0 6px var(--accent-glow)' }}
                      />
                    </div>
                  </div>
                ))}
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div
      className="p-3 rounded transition-all border border-transparent hover:border-[var(--accent-primary)]"
