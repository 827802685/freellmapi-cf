...
import { IconRefresh, IconTrash, IconChevronDown, IconChevronRight, IconEdit } from '../components/Icons';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-semibold gradient-text">{t('settings.title')}</h1>
...
function AddProviderForm({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <div className="card-gradient-border space-y-3" style={{ borderColor: 'var(--accent-primary)' }}>
    ...
    <div className="card galaxy-fade-in">
      ...
      >
        <div className="flex items-center gap-3">
          <span className="text-text-muted inline-block transition-transform" style={{ transform: expanded ? 'rotate(0deg)' : 'rotate(0deg)' }}>
        ...
        <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
          ...
          >
            {deleting ? <span className="galaxy-spinner" style={{ width: 14, height: 14, borderWidth: 2 }} /> : <IconTrash size={14} />}
          </button>
        </div>
      </div>
...
function ModelsTable({
  ...
  return (
    <div className="space-y-2">
      ...
      {selected.size > 0 && (
        <div
          className="flex items-center gap-3 rounded-md p-2 alert-danger"
        >
...
function ModelRow({
  ...
  return (
    <tr
      ...
      style={{ opacity: model.enabled ? 1 : 0.55 }}
    >
      <td className="py-2 px-2 text-center">
