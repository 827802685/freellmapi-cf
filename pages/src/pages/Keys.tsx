...
import { IconRefresh, IconEye, IconEyeOff, IconTrash } from '../components/Icons';
...
export function KeysPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ===== 已添加的 keys 列表(可折叠) ===== */}
      <div className="card galaxy-fade-in">
        ...
        {showList && (
          keysLoading ? (
            <div className="text-center py-6 text-text-secondary text-sm flex items-center justify-center gap-2">
              <span className="galaxy-spinner"></span>
              {t('common.loading')}
            </div>
          ) : keys.length === 0 ? (
...
const PROVIDER_OPTIONS = [
  ...
  { value: 'aihorde', label: 'AI Horde (anon)' },
  { value: 'bailian', label: '阿里云百炼 (Bailian)' },
    ...
    <RevealModal revealed={revealed} onClose={() => setRevealed(null)}>
      <div className="card galaxy-fade-in">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {err && (
            <pre className="text-xs text-danger whitespace-pre-wrap break-words rounded p-2 alert-danger">
              {err}
            </pre>
          )}
...
function CustomProviderKeyCard({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <RevealModal revealed={revealed} onClose={() => setRevealed(null)}>
      <div className="card galaxy-fade-in" style={{ borderColor: 'var(--accent-primary)' }}>
      ...
      {revealed && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="card max-w-lg w-full">
            ...
            <div className="rounded-md p-3 text-sm font-mono break-all mb-4 alert-warning">
              {revealed.keyPlain}
            </div>
    ...
    <>
      <tr className="border-b border-border-subtle hover:bg-bg-tertiary">
        ...
        <td className="py-2 px-2 text-right">
          <button className="icon-btn icon-btn-sm" onClick={reveal} disabled={busy} title={t('keys.action.show.short')}>
            {revealing ? <IconEyeOff size={14} /> : <IconEye size={14} />}
          </button>
          <button className="icon-btn icon-btn-sm" onClick={check} disabled={busy} title={t('keys.action.check')}>
            <IconRefresh size={14} />
          </button>
          <button className="icon-btn icon-btn-sm icon-btn-danger" onClick={remove} title={t('keys.action.delete')}>
            <IconTrash size={14} />
          </button>
        </td>
      </tr>
