...
export function AnalyticsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold gradient-text">{t('analytics.title')}</h1>
      ...
      {/* 最近请求 */}
      <div className="card galaxy-fade-in overflow-hidden p-0">
            ...
            {recent.length === 0 ? (
              ...
              <>
                {(showAllRecent ? recent : recent.slice(0, 10)).map((l: any) => (
                  <tr key={l.id} className="border-b border-border-subtle hover:bg-bg-tertiary">
...
function StatCard({ label, value }: { label: string; value: any }) {
  return (
    <div className="card-gradient-border !p-4 transition-all hover:-translate-y-0.5 hover:[box-shadow:0_0_16px_var(--accent-glow)]">
      ...
      {data.map(d => (
        <div key={d.label} className="text-sm">
          ...
          <div className="rounded-full h-2 overflow-hidden" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
            <div className="h-full transition-all" style={{ width: `${(d.value / max) * 100}%`, backgroundColor: color, boxShadow: `0 0 8px ${color}` }} />
          </div>
        </div>
      ))}
    </div>
