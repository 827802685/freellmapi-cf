...
tokensRoute.get('/', async (c) => {
  ...
  ).all<{ id: number; token_hint: string; label: string | null; enabled: number; created_at: number; last_used_at: number | null; request_count: number }>();
  // 转成 camelCase 给前端
...
tokensRoute.get('/:id/plain', async (c) => {
  ...
  try {
    ...
    return ok(c, { error: 'decrypt_failed', message: e instanceof Error ? e.message : String(e) }, 500);
  }
});
